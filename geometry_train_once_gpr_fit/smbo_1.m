...
while (k <= maxiter)
    ...
    switch(m.surrogate)
        case 'TPS-RBF'      % Radial-Basis Function (Thin-Plate Spline)
            ...
            [xopt,fopt] = ga(@(x)objective(x,xs_g,fsmp),pc.nvar,...
                [],[],[],[],pc.lb,pc.ub,@(x)cons(x, xs_g_con1, fsmp_con1),opt);
...
z1=[xopt];
save('optimal_solution_sub_2.mat', 'z1');
save('surrogate_response_sub_1.mat', 'Mdl_obj_1');
end
