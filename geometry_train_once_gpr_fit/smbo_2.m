...
while (k <= maxiter)
    ...
    if (k>1)
        for i = 2:k
            xs_g_con1 = [xs_g_con1; x_sample{i}];
            fsmp_con1 = [fsmp_con1; f_hf_con1{i}];
        end
    end
    %%
    ...
    if (k>1)
        for i = 2:k
            xs_g_con2 = [xs_g_con2; x_sample{i}];
            fsmp_con2 = [fsmp_con2; f_hf_con2{i}];
        end
    end
    ...
    switch(m.surrogate)
        case 'TPS-RBF'      % Radial-Basis Function (Thin-Plate Spline)
            % Construct an RBF model using thin plate spline function
               ...
               [xopt,fopt] = ga(@(x)objective_2(x,xs_g,fsmp),pc.nvar,...
                [],[],[],[],pc.lb,pc.ub,@(x)cons_2(x, xs_g_con1, fsmp_con1,xs_g_con2, fsmp_con2),opt);
...
save('surrogate_response_sub_2.mat', 'Mdl_obj_2');
end
