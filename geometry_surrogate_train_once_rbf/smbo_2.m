...
while (k <= maxiter)
    ...
    switch(m.surrogate)
        case 'TPS-RBF'      % Radial-Basis Function (Thin-Plate Spline)
            ...
            [weight_2,center_2] = tps_rbf_construct2(xs_g,fsmp);
            [weight1,center1] = tps_rbf_construct2( xs_g_con1, fsmp_con1);
            [weight2,center2] = tps_rbf_construct2( xs_g_con2, fsmp_con2);
            % Run GA-based optimization using the surrogate model
            [xopt,fopt] = ga(@(x)tps_rbf_objfn2(x,weight_2,center_2),pc.nvar,...
                [],[],[],[],pc.lb,pc.ub,@(x)tps_rbf_confn2(x,weight1,center1,weight2,center2),opt);
...
%
 %   x_table=store{i};
  %  x_table_input=x_table(:,3:4);

    %   no_row=size(x_table_input,1);
    %   opt_sol=repmat(optimal{i},no_row,1);

     %  reconfiguredCell = [reconfiguredCell; store{i}(:, 3:4)];
